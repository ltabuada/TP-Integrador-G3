...
delimiter ;
call crearPublicacionSubasta(11,11,current_date(),25000);
...
delimiter ;
call crearPublicacionVentaDirecta(11,11,11,10);
 
delimiter //
create procedure crearPublicacionp (in id int, in precio float, in nivel varchar(45), in estado varchar(45), in producto_id int, in categoria_id int, in usuario_dni int, in fecha date, in tipo varchar(20), in oferta int, in fecha_tope datetime, in oferta_minima int, in pago_id int, in envio_id int, in calificacion int)
begin
  insert into publicacion values (id, precio, nivel, estado, producto_id, categoria_id, usuario_dni, fecha);
  if tipo = "subasta" then
    call crearPublicacionSubasta(oferta, id, fecha_tope, oferta_minima);
  elseif tipo = "venta_directa" then
    call crearPublicacionVentaDirecta(pago_id, envio_id, id,calificacion);
  end if;
end//
delimiter ;
drop procedure crearPublicacionp;
call crearPublicacionp(20, 2500.75, "premium", "Finalizada", 1, 1, 1, "2025-06-27", "subasta", 1000, "2025-07-15 23:59:59", 900, null, null, null);
...
if (select estado from publicacion where id = idPubli) = "Finalizada" then
set mensaje = "La publicacion no esta activa";
else if (select subasta.publicacion_id from subasta where publicacion_id = idPubli) != null then
set mensaje = "Es una subasta";
else if idPubli in (select id from publicacion) then
insert into compra values(17, current_date(), 7, dni, idpubli, metPago, envio);
set mensaje = "Compra exitosa";
else
set mensaje="La publicacion no existe";
end if;
end if;
end if;
return mensaje;
...
select comprarProducto(1,5,7,1);
...
if (select calificacion_compra from compra where usuario_DNI = DNI limit 1) is not null then
delete from publicacion where usuario_DNI = DNI;
update publicacion set estado = "Finalizada";
set msg = "Publicacion cerrada";
else
...
update publicacion set estado = "Pausada" where id = idPubli;
...
begin
    ...
    select count(*) into cant_publicaciones from publicacion where id = idDeseado and estado = "Activa";
	...
	if ofertaNueva > ofertaActual then
		insert into historial_ofertas values(null,ofertaNueva, usuarioOfertante, idDeseado);
        update subasta set oferta = ofertaNueva where publicacion_id = idDeseado;
...
select pujarProducto(3,900000,1);
#6)------------------------------------------------------------------------------------------------
delimiter //
create function eliminarCategoria (categoriaDeseada varchar(45)) returns text deterministic
begin
	declare mensaje text default "";
    declare idCategoriaDeseada int default 0;
    select idCategoria into idCategoriaDeseada from categoria where categoria.categoria = categoriaDeseada;
    if not exists (select * from publicacion where categoria_idCategoria = idCategoriaDeseada) then
...
insert into categoria values(12,"telas");
select eliminarCategoria("telas");
...
begin 
declare mensaje text default "";
declare dniDeseado int default 0;
select publicacion.usuario_DNI into dniDeseado from publicacion join preguntas on publicacion.id = publicacion_id where idPregunta = idPreg  order by publicacion.usuario_DNI asc limit 1;
if id_vendedor = dniDeseado then
	insert into respuestas values(null, msgRespuesta, idPreg);
    set mensaje = "Pregunta Respondida";
else 
	set mensaje = "Este usuario no puede responder";
	end if;
return mensaje;
end //
delimiter ;
drop function responderPregunta;
select responderPregunta(1,1,"claro");
...
begin
delete from historial_ofertas where publicacion_id=old.id;
delete from subasta where publicacion_id=old.id;
delete from venta_directa where publicacion_id=old.id;
delete from compra where publicacion_id=old.id;
 
  delete from respuestas where preguntas_idPregunta in (select idPregunta from preguntas where publicacion_id = old.id);
...
begin
    ...
    set estado = "observadas" where publicacion.estado = "activo" and metodo is null;
end //
delimiter ;
...
#1)
delimiter //
create procedure crearPublicacion (in idPubli int, in cuantoDuele float, in nivelPubli varchar(45), in estado varchar(45),in idProdu int, in idCat int, in DNI int, in fecha datetime)
begin 
start transaction;
		insert publicacion values(idPubli,cuantoDuele,nivelPubli,estado,idProdu,idCat,DNI,fecha);
 if (select current_date())<=fecha then
        commit;
        select "Publicación creada con éxito." as mensaje;
    else
        rollback;
        signal sqlstate '45000' set message_text = "La fecha de la publicación es antigua.";
    end if;
end//
delimiter ;

call crearPublicacion(12,1000,"Oro","activa",6,6,10,"2025-1-1");
